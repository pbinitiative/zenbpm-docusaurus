...
Deploy it by sending the file as the request body:

```bash
curl -X POST http://localhost:8080/v1/process-definitions \
  -H "Content-Type: application/octet-stream" \
  --data-binary @first-bpmn-process.bpmn
```
...
- **The APIs in full.** [REST (OpenAPI)](../static/openapi.mdx) and the [gRPC proto](pathname:///zenbpm-v1.8.0.proto).
